...
#
#    ⚠️ 몫을 380 → 320 으로 줄였다. 아래 (1b) 운행대수 수집에 자리를 내주기 위해서다.
#    관측상 일일 한도가 약 2,000 이라 5회 × 320 = 1,600 에 대수 5 × 60 = 300 을
#    더해 1,900 이다. 속도는 전 노선을 한 바퀴 도는 데 하루쯤 더 걸리지만,
#    시간대별 배차를 아예 모르는 것보다 낫다.
Step '서울 구간속도 스냅샷' @('--mode','seoulspeed','--limit','320')

# 1b) 운행 대수 — **시간대별 배차**를 재는 유일한 길이다.
#
#     어떤 공개 API 도 시간대별 배차를 안 준다. 우리 버스 배차는 노선당 하루 한
#     값이고(서울 term, 경기 GBIS 주중배차간격), 정작 이 제품이 보는 건 출근
#     시간대다. 차량 위치 API 로 "지금 몇 대 도는지"를 세면
...
#     승차 여유가 역방향에서 엉뚱한 곳(하차 뒤)에 붙어 출근 도달권을 흔들고
#     있었는데, 구간 1,118개 대조에서 아무 표시도 안 났다. 자세한 건 ADR-36.
Step '불변식 검사' @('--mode','invariants','--samples','120')
...
#
#    한 번에 **약 24분**(출발지 621 × 슬롯 20 × 표본 5 = 62,100회 탐색.
#    실측 2026-09-09: 250개에 602초 = 출발지당 2.4초)이라 매 회차 돌릴 이유가 없다. 바뀌는 건 속도
#    스냅샷이 하나 더 쌓였을 때뿐이고, 그건 하루 다섯 번 중 몇 번이다.
#    ⚠️ 수정시각으로 비교하면 안 된다. `--mode gtfs` 와 `--mode gtfssubway` 는 내용이
#    같아도 매번 zip 을 새로 쓰므로 zip 이 항상 더 새것이 되고, 행렬이 매 회차 헛돈다.
#    **내용 해시**로 본다 — 그래서 두 zip 의 항목 시각을 고정해 뒀다(SubwayGtfs.FIXED_TIME).
