...
class Raptor(private val d: TransitData) {
    ...
    companion object {
         ...
         * 승차 횟수 상한(= 환승 상한 + 1).
         *
         * **처음엔 5 였고, 재본 적이 없었다.** `maxPerStop` 을 64 로 풀어 도보 환승을
         * 포화시킨 뒤 승차 상한만 올려보니(출발지 8곳 · 도착 08:00):
         *
         * ```
         * 6회 → 도달 쌍의 1.82% 가 빨라짐 (중앙 −3분 · 최대 −18분) · 새로 닿음 23곳
         * 7회 →           1.95%                                   · 새로 닿음 27곳
         * ```
         *
         * **6 에서 거의 포화한다** — 7 이 더 주는 건 0.13%p 뿐이다. 그래서 6 으로 둔다.
         * 대가는 배치 214초 → 247초.
         *
         * ⚠️ 이건 성능 상한이자 **모델링 판단**이기도 하다. RAPTOR 는 최단시간을 고르므로
         * 승차 6회 경로는 그게 실제로 더 빠를 때만 이긴다. 다만 "환승 5번 해서 3분 빠른
         * 길"을 사람이 실제로 택하느냐는 우리가 모델링하지 않는 문제로 남는다.
         *
         * `var` 인 이유는 `--rounds` 로 다시 재보기 위해서다. 평상시엔 건드리지 않는다.
         */
        var MAX_ROUNDS = 6
