...
object Gtfs {
    ...
    /** 정류장에 서는 시간. `sectSpd` 는 주행 속도라 정차가 안 들어 있다. */
    private const val DWELL_SEC = 20

    /**
     * 경기 구간 거리 보정.
     *
     * 경기 API 는 구간 거리를 안 준다. 좌표 직선거리로 대신하는데, 서울 575구간에서
     * 실제 도로거리 ÷ 직선거리의 중앙값을 재보니 **1.01** 이었다. 정류장 간격이
     * 짧아서(중앙값 400m) 도로가 굽을 여지가 별로 없다. 그래서 이 보정으로 충분하다.
     */
    private const val DETOUR = 1.01
